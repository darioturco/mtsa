...
def graph_training_process(sliding_window=5, save_path=None, use_steps=False):
    ...
    for instance in BENCHMARK_PROBLEMS:
        data_instance = pd.read_csv(f"./results/training/{instance}-2-2-partial.csv", names=['Step', "Reward", "Loss"])   # Cambiarle el nombre a {instance}-2-2
        ...
        if graph_loss:
            plt.clf()
            plt.plot(x[:limite], losses[:limite], label="Loss")
            plt.xlabel(x_label)
            plt.ylabel('Loss')
            plt.title(instance)
            plt.show()
