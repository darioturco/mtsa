...
public class DirectedControllerSynthesisNonBlocking<State, Action> extends DirectedControllerSynthesis<State, Action> {
    ...
    public static HeuristicMode mode = HeuristicMode.TrainedAgent;
    ...
    private List<Set<State>> canReachMarkedInLts;
            ...
            HashMap<Integer, Integer> assumptions)
    {
        ...
        while (heuristic.somethingLeftToExplore() && !isFinished()) {
            ...
            if(expansion_budget==0) {
                statistics.informExceededBudget();
                statistics.end();
                return null;
            }
    ...
    public void setHeuristic(ExplorationHeuristic<State, Action> heuristic){
        if(heuristic == null) {
            ...
            if (mode == HeuristicMode.TrainedAgent) {
                DCSFeatures<State, Action> featureMaker = new DCSFeatures<>(null, null, 1000000, null);
                this.heuristic = new FeatureBasedExplorationHeuristic<>("random", featureMaker, false);
            } else {
    ...
    private Compostate<State, Action> buildCompostate(List<State> states, Compostate<State, Action> parent) {
        ...
        if (result == null) {
            ...
            statistics.startHeuristicTime();
            heuristic.newState(result, parent, states);
            statistics.endHeuristicTime();
    ...
    private void open(Compostate<State, Action> parent, HAction<State, Action> action, Compostate<State, Action> child) {
        ...
        assertTrue("Attempted to expand from goal or error state", isNone(parent));
    ...
    private void findNewGoals() {
        ...
        Set<Compostate<State, Action>> c = new LinkedHashSet<>(loop);
            ...
            while(!reached_fixpoint){
                ...
                while (it.hasNext()) {
                    ...
                    if(forcedByEnvironmentToLeave(state, c)){
                        ...
                        if(!hasParentInC){
                            /* States removed this way are not notifiedAsNone to heuristic because they are not added to open.
                               This states are added to open later if we have no conclusions for them when propagating the ones
                               that are forced to leave.
    ...
    private void propagateGoal(Set<Compostate<State, Action>> newGoals, Set<Compostate<State, Action>> parent) {
        ...
        heuristic.notifyPropagatingGoal(c);
    ...
    public boolean forcedByEnvironmentToLeave(Compostate<State, Action> state, Set<Compostate<State, Action>> targets) {
...
//        assertGoodDirector(result, directStates, directMarked);
    ...
    // TODO: This method should always be used, and transitions should be managed by DCS instead of heuristics
    // This method removes controllable transitions in components that go to states from which it is impossible to reach a marked state
    // It also computes the sets of states that can reach marked states, to be used with uncontrollable actions during exploration
    public void analyzeReachability(){
        canReachMarkedInLts = new ArrayList<>();
        for (int i = 0; i < ltss.size(); i++) {
            // Note: We could do only one bfs starting from all marked states, but inverting the LTS is surprisingly expensive.
            Set<State> canReach = new HashSet<>();
            for(State src : ltss.get(i).getStates()){
                Set<State> visited = new HashSet<>();
                LinkedList<State> q = new LinkedList<>();
                q.add(src);

                boolean reachedMarked = false;
                while(!q.isEmpty() && !reachedMarked){
                    State s = q.getFirst();
                    q.removeFirst();
                    for(Pair<Action, State> p : ltss.get(i).getTransitions(s)){
                        State t = p.getSecond();
                        if(defaultTargets.get(i).contains(t)) {reachedMarked = true; break; }
            ...
            canReachMarkedInLts.add(canReach);
        }
        for(int i = 0; i < ltss.size(); i++){
            for(State s : ltss.get(i).getStates()){
                int finalI = i;
                ltss.get(i).getTransitions(s).removeIf(t -> !canReachMarkedInLts.get(finalI).contains(t.getSecond()) && controllable.contains(t.getFirst()));
            }
        }
    }
