...
public class Analyser implements Animator, Automata {
    ...
    /*
    private int getStateForComposite(int actionNo, int currentState, CompactState composition) {
        EventState es = composition.states[currentState];
        while (es != null) {
            if (es.event == actionNo) {
                return es.next;
            } else {
                es = es.list;
            }
